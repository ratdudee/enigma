...
int load_config_from_args(int argc, char* argv[], Config* cfg, int* do_encrypt) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "-i") == 0 && i + 1 < argc) {
            ...
            reflector = argv[++i];
        } else if (strcmp(argv[i], "--wheels") == 0 && i + 1 < argc) {
            wheels = argv[++i];
            ...
            fprintf(stdout, "Usage: %s -i input.txt (-o output.txt) (-k keyfile | --reflector R --wheels W --rings R) [-d]\n", argv[0]);
            return -2;
        } else {
    ...
    if (keyfile) {
        ...
        // must at least provide wheels/rings/reflector via args
        cfg->key = parse_key_components(reflector, wheels, rings, plugboard);
